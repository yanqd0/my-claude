...
3. **归类改动**：不查看无关代码，据 commit message 与上述变动归为三类：
   ...
   - **语义合并与拆分**：CHANGELOG 面向用户，commit 面向开发者，两者**不必一一对应**。
     - **合并**：多个相关 commit（如同一功能的渐进实现、迭代修复、多次微调）合并为一条 CHANGELOG 条目；同类杂项改动（如多处文档修正、多个小重构）可合并为一条概括性条目（如"文档补全与措辞修正"）。
     - **拆分**：一个复杂 commit 涉及多个独立模块的改动，拆为多条 CHANGELOG 条目，按功能/模块各归其类。
     - **fix 吞噬**（既有规则）：同版本内 feat 的实现过程中产生的 bugfix，被 feat 条目吸收，不再单独列出。此外，refactor/docs/ci 等 Others 类 commit 若完全服务于某个 feat/fix，也被该条目吸收。
...
**分组与二级列表**：当某个分类下条目较多（≥5 条）或改动涉及多个可辨识的功能/模块时，按功能或模块分组，用一级条目概述、二级列表展开细节：
...
分组原则：
- 一级条目保持一句话总结变化；二级条目补充细节，每条也尽量用一句话。
- 不可分组的零散条目保持平铺一级列表即可，不与分组条目混在同一分类下——要么全分组，要么全平铺。
- 简单条目（一句话已说清）不需要强行展开二级列表。
