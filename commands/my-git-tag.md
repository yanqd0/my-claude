...
4. 生成 tag message：
   - 通过 `git log --oneline <上一个tag>..HEAD` 获取区间内所有 commit message。
...
6. 提交 CHANGELOG.md：
   ...
   - 执行 `git commit -m "Update CHANGELOG.md for <version>"`
7. 执行 `git tag -a <tag_name> -m "<message>"`。tag 打在第 6 步新产生的提交上。
